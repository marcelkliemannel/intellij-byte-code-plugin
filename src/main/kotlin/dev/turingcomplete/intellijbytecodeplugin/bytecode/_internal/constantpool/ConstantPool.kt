...
internal class ConstantPool(val entries: List<ConstantPoolInfo>) {
  ...
  companion object {
    ...
    private val TAG_INFO_CREATORS =
      mapOf<Int, (DataInputStream) -> ConstantPoolInfo>(
    ...
    fun create(classFile: ClassFile): ConstantPool {
      DataInputStream(classFile.file.inputStream).use { dataInputStream ->
        ...
        while (index <= constantPoolCount - 1) {
          ...
          val constantPoolInfo =
            TAG_INFO_CREATORS[tag]?.invoke(dataInputStream)
              ?: throw IllegalStateException(
                "Unknown constant pool entry tag: $tag (already read $index out of ${constantPoolCount - 1})"
              )
  ...
  /**
   * The constant pool starts at index one, but the underling structure used in this class at 0. To
   * correctly resolve referenced indices from entries this method must be used.
   *
   * If its required to work on the 0-indexed structure, the property [entries] is to be used.
   */
