...
object MethodFramesUtils {
  ...
  fun collectFrames(method: MethodNode, owner: ClassNode): MethodFrames {
    ...
    val methodFrames =
      (0 until method.instructions.size())
        .asSequence()
        .map { i ->
          ...
          if (frame != null) {
            val stack =
              (0 until frame.stackSize).mapNotNull { j -> frame.getStack(j).type }.toTypedArray()
            val locals =
              (0 until frame.locals).mapNotNull { j -> frame.getLocal(j).type }.toTypedArray()
