...
internal class AnalyzeByteCodeAction :
  ...
  override fun update(e: AnActionEvent) {
    e.presentation.isEnabledAndVisible =
      CommonDataKeys.PROJECT.getData(e.dataContext)?.let { project ->
        ...
        if (files?.any { ClassFilesFinderService.fileCanBeAnalysed(it, project) } == true) {
          ...
          findPsiElement(project, e.dataContext).let { result ->
            ...
            if (containingFile != null) ClassFilesFinderService.fileCanBeAnalysed(containingFile)
            else false
          }
        }
      } ?: false
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    if (psiElement != null) {
      project
        ...
        .openPsiElements(mapOf(psiElement to editorPsiFile))
      return
    }
  ...
  private fun findPsiElementInInjectedEditor(
    ...
    return InjectedLanguageManager.getInstance(project)
      .findInjectedElementAt(editorPsiFile, offset)
      ?.containingFile
