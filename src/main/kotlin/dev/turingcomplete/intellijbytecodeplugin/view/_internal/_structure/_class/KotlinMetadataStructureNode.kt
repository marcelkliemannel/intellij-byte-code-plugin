...
internal class KotlinMetadataStructureNode(
  ...
  init {
    asyncAdd(true) {
      fieldNameToValueList
        ...
        .map { it.first to it.second }.forEach { (fieldName, value) ->
          when (fieldName) {
  ...
  private fun addVersionNode(preFix: String, value: Any?) {
    add(
      ValueNode(
        preFix = preFix,
        displayValue = value.asSafely<ArrayList<Int>>()?.joinToString(".") ?: value?.toString() ?: "null"
      )
    )
  }

  private fun addDataNode(title: String, value: Any?) {
    val values = value.asSafely<ArrayList<String>>()?.map {
      if (it.isNotBlank()) {
  ...
  private fun addExtraIntNode(value: Any?) {
    ...
    val values = extraIntMappings.mapNotNull { (bit, contextHelp) ->
  ...
  companion object {
    ...
    private val kotlinIcon =
      IconManager.getInstance().getIcon("dev/turingcomplete/intellijbytecodeplugin/icon/kotlin.svg", ClassStructureNode::class.java.classLoader)

    private val extraIntMappings = mapOf(
      1 to "Multi-file class facade or part",
      2 to "This class file is compiled by a pre-release version of Kotlin and is not visible to release versions",
      3 to "Strict metadata version semantics",
      4 to "This class file is compiled with the new Kotlin compiler backend (JVM IR) introduced in Kotlin 1.4",
      ...
      7 to "This class is used in the scope of an inline function and implicitly part of the public ABI"
    )
  }
}