...
class InstructionsOverviewTool : ByteCodeTool("Instructions Overview") {
  ...
  companion object {
    ...
    private val COLUMNS =
      Vector(listOf("Instruction", "Opcode", "Operands", "Stack modification", "Description"))
  ...
  private class InstructionsTable : JBTable(InstructionsTableModel()), DataProvider {
    ...
    init {
      ...
      rowSorter = TableRowSorter(model).apply { setComparator(1, OPCODE_COMPARATOR) }

      addMouseListener(
        UiUtils.Table.createContextMenuMouseListener(
          InstructionsOverviewTool::class.java.simpleName
        ) {
  ...
  private class InstructionsTableModel : DefaultTableModel() {
    ...
    private fun readInstructionsData(): Vector<Vector<String>> {
      ...
      val separator = '|'
      val dataCsv =
        InstructionsOverviewTool::class
          .java
          .getResourceAsStream(
            "/dev/turingcomplete/intellijbytecodeplugin/byte-code-instructions.csv"
          ) ?: throw IllegalStateException("snh: byte-code-instructions.csv missing")
      return Vector(
        InputStreamReader(dataCsv, "UTF-8").useLines {
          it
            .map { line ->
              // Part of the code comes from
              // https://github.com/JetBrains/intellij-community/blob/master/platform/platform-util-io/src/com/intellij/execution/process/impl/CSVReader.java
              ...
              while (i < line.length) {
                ...
                if (c == quotechar) {
                  ...
                  if ( // there is indeed another character to check.
                    inQuotes && line.length > i + 1 && line[i + 1] == quotechar
